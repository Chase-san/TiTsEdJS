...
(async function() {
  ...
  const titsed = async function () {
    ...
    const right_arrow = "&#9654;"; //used for hide button
    const left_arrow = "&#9664;"; //used for show button
    ...
    show_hide_button.onclick = function () {
      if (editor_shown) {
        ...
        editor_shown = false;
        //do hide
        editor_app.style = `right: -${editor_width}px`;
      } else {
    ...
    function repopulateBreastsTable() {
      ...
      for (var i = 0; i < game_app.pc.breastRows.length; ++i) {
        ...
        table.appendChild(createFlagRow("Areola Flags", game_app.pc.breastRows[i], "areolaFlags", `br${i}_areola_flag`, game_app.GLOBAL.FLAG_NAMES, VALID.FLAGS.AREOLA));
      }
      table.appendChild(
